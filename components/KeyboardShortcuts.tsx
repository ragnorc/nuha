 ...
 *
 * The reading column reserves a right gutter for this (see layout.tsx), so the
 * sentence detail can grow - expanded grammar notes included - without ever
 * running underneath it.
 */
export function KeyboardShortcuts({ sticky = false }: { sticky?: boolean }) {
  return (
    <div className="fixed bottom-8 right-10 z-20 flex flex-col gap-2">
...
function ShortcutKey({
  ...
  return (
    <div className={`flex items-center ${className}`}>
      <kbd
        className={`flex items-center justify-center w-7 h-7 shrink-0 text-xs font-bold text-white rounded-lg shadow-[inset_0_-3px_0_#2D3444,inset_0_-4px_0_#5A6374,0_5px_8px_rgba(0,0,0,0.25),0_0_1px_#131720] border border-b-0 ${
          highlighted
